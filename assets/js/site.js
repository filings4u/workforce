document.addEventListener("DOMContentLoaded",()=>{
  ...
  function select(target){audience=target;buttons.forEach(b=>b.classList.toggle('active',b.dataset.pricingTab===target));groups.forEach(g=>g.hidden=g.dataset.priceGroup!==target);renderSticky();updateSticky()}
  ...
  function updateSticky(){if(!sticky)return;const top=document.getElementById('pricingTopAnchor'),end=document.getElementById('pricingComparisonEnd');if(!top||!end)return;const topY=top.getBoundingClientRect().top,endY=end.getBoundingClientRect().top;const show=topY<90&&endY>window.innerHeight*.72;sticky.classList.toggle('is-visible',show);sticky.setAttribute('aria-hidden',String(!show))}
